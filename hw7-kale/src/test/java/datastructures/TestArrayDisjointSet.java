...
public class TestArrayDisjointSet extends BaseTest {
    ...
    @Test(timeout=SECOND)
    public void testMakeSetValue() {
        IDisjointSet<String> test = new ArrayDisjointSet<>();
        test.makeSet("a");
        assertEquals(test.findSet("a"), 0);
        test.makeSet("b");
        assertEquals(test.findSet("b"), 1);
    }

    @Test(timeout=SECOND)
    public void testDuplicateUnion() {
        IDisjointSet<String> test = new ArrayDisjointSet<>();
        test.makeSet("a");
        test.makeSet("b");
        test.union("a", "a");
        assertTrue(test.findSet("a")!=test.findSet("b"));
        test.union("a", "b");
        assertEquals(test.findSet("a"), test.findSet("b"));
    }
